...
async function getmoreornot() {
    const token = localStorage.getItem('access_token');
    const post_user_id = localStorage.getItem('post_user_id');
    try {
        const response = await fetch( `http://127.0.0.1:8000/friends/dualside/${post_user_id} `, {
            method: 'GET',
            headers: {
...
try {
    ...
    else if(typeoffile == 'contact'){
        return `${data.contact_number}`;
    }
    else if(typeoffile == 'address'){
        return `${data.address}`;
    }
    else if(typeoffile == 'birthday'){
...
async function fetchPosts() {
    const token = localStorage.getItem('access_token');
    const userId = localStorage.getItem('post_user_id');
    if (!token) {
        Toastify({
            text: 'You are not logged in. Redirecting to login page.',
            duration: 3000,
...
async function fetchUserInfo() {
    ...
    try {
        ...
        value = await getmoreornot();
        if (value){
            const element = document.getElementById('account');
            element.innerHTML = `<p class="info"> Contact number : ${await getimageloction("contact",user.id)}</p>
            <p class="info"> Address :  ${await getimageloction("address",user.id)}</p>
            <p class="info"> Birthday :  ${await getimageloction("birthday",user.id)}</p>`;
        }
        else{
            const element = document.getElementById('account');
            element.classList.remove('account-container-1');
        }
...
async function status_load() {
    ...
    try{
        ...
        const response = await fetch(`http://127.0.0.1:8000/friends/${userId}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
document.querySelector('.follow-button').addEventListener('click', async function() {
    try{
        if (user_fol){
            ...
            const token = localStorage.getItem('access_token');
            const userfId = localStorage.getItem('post_user_id');
            ...
            body: JSON.stringify({ friend_id: userfId })
            });
            if (!response.ok) {
                throw new Error('Failed to fetch user info. Please log in again.');
            }
