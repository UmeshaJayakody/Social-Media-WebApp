...
document.querySelectorAll('.profile-button').forEach(button => {
    button.addEventListener('click', function() {
        const post_userId = this.getAttribute('data-user-id');
        localStorage.setItem('post_user_id', post_userId);
        if (post_userId === localStorage.getItem('user_id')) {
...
async function votePost(postId, dir) {
    ...
    try {
        ...
        if (dir === 1) {
            ...
            likeButton.setAttribute('onclick', `votePost(${postId}, 0)`); // Update to allow removing the vote
        } else {
